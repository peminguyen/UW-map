...
public class TextInterfaceView {
     ...
     */
    public String showPath(String start, String end, Path<Point> path) {
        String s = "";
        s += "Path from " + start + " to " + end + ":\n";
        for(Path<Point>.Segment pathSegment : path) {
            ...
            s += "\tWalk " + Math.round(pathSegment.getCost()) + " feet " +
                    dir.name() + " to (" + Math.round(pathSegment.getEnd().getX()) + "," +
                    Math.round(pathSegment.getEnd().getY()) + ")\n";
        }
        s += "Total distance: "+ Math.round(path.getCost()) + " feet\n";
